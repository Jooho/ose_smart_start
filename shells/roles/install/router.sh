...
echo "oadm ca create-server-cert --signer-cert=$CA/ca.crt  --signer-key=$CA/ca.key --signer-serial=$CA/ca.serial.txt --hostnames=\"*.${subdomain}\"  --cert=$CA/router.crt --key=$CA/router.key"
...
cat $CA/router.crt $CA/router.key $CA/ca.crt > $CA/router.pem
